...
bench_associated() {
    ...
    echo_action "Ready to receive associate flows."
    echo_action "Press ENTER to stop gather CPU statistics..."

    /usr/bin/python3 $bench_tools_dir/bench_cpu.py --coflowiness=$1 --flow_size_distribution=$2 --bench-type=associated
    kill -2 "$print_netspeed_pid"  # Send SIGINT to netspeed function

    echo_action " "
    echo_action "Done receiving associate flows."
}
...
bench_base() {
    ...
    /usr/bin/python3 $bench_tools_dir/bench_cpu.py --coflowiness=$1 --flow_size_distribution=$2 --bench-type=base
    kill -2 "$print_netspeed_pid"  # Send SIGINT to netspeed function
...
bench_base $coflowiness $flow_size_distribution
